...
class FileWatcherService:
    ...
    async def trigger_tactical_sync(self, project_id: str, batch: set[str]):
        _logger.info(f"[Watchdog] Tactical Sync (Incremental): {len(batch)} files in project {project_id}")
            ...
            try:
                ...
                for callback in self._on_change_callbacks:
                    try:
                        ...
                        _logger.error("Error in watcher callback for project=%s file=%s", project_id, filepath, exc_info=True)
                ...
                _logger.error("Error in vector_sync_worker for project=%s", project_id, exc_info=True)
