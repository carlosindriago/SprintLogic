...
PATTERNS: list[tuple[str, str]] = [
    ...
    ("typescript", r"^\s*(?:export\s+)?(?:default\s+)?(?:abstract\s+)?(?:class|function|const|let|var)\s+([a-zA-Z0-9_$]+)"),
    ...
    ("rust", r"^\s*(?:pub\s+)?(?:async\s+)?(?:unsafe\s+)?(?:fn|struct|enum|trait|impl|mod|const|static)\s+([a-zA-Z0-9_]+)"),
    # Go: func, type, var, const
    ("golang", r"^\s*(?:func|type|var|const)\s+(?:\([^)]*\)\s+)?([a-zA-Z0-9_]+)"),
    # Java/PHP: class, interface, enum, function
    ("other", r"^\s*(?:(?:public|private|protected|static|final|abstract)\s+)*(?:class|interface|enum|function)\s+([a-zA-Z0-9_]+)"),
]
