...
class SemanticMarkdownSplitter:
    ...
    def __init__(
        ...
        hierarchy_template: str = "[Documento: {doc}] [Sección: {breadcrumbs}]\n\n",
    ):
    ...
    def split(self, content: bytes, file_name: str) -> list[str]:
        if not self.parser:
            return [
                self._apply_seal(
                    content.decode("utf-8", errors="ignore")[: self.max_chunk_size], file_name, []
                )
            ]
            ...
            if len(current_chunk_text) + len(node_text) > self.max_chunk_size:
                ...
                if len(node_text) > self.max_chunk_size:
                    ...
                    if current_chunk_text.strip():
                        chunks.append(
                            self._apply_seal(current_chunk_text, file_name, current_breadcrumbs)
                        )
                        current_chunk_text = ""

                    # Aplicar la válvula de escape Godzilla
                    godzilla_chunks = self._split_godzilla_node(
                        node, content, file_name, current_breadcrumbs
                    )
    ...
    def _split_godzilla_node(
        self, node, content: bytes, file_name: str, breadcrumbs: list[str]
    ) -> list[str]:
        """
