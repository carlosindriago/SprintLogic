...
class TddGuardValidator:
    ...
    def get_future_state_tests(self) -> list[tuple[str, bytes]]:
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "ls-files"], cwd=self.project_path, capture_output=True, text=True
            )
            ...
            test_files = [
                f
                for f in all_files
                if ("test" in f or "spec" in f) and f.endswith((".ts", ".tsx", ".py", ".js"))
            ]
    ...
    def validate_task_from_memory(
        self, task_id: str, files_in_memory: list[tuple[str, bytes]]
    ) -> tuple[bool, list[str]]:
        """
                ...
                if not docblock and node.type == "function_definition":
                    ...
                    if (
                        ...
                        if first_expr.children and first_expr.children[0].type == "string":
                            docblock = code[first_expr.start_byte : first_expr.end_byte].decode(
                                "utf-8"
                            )
