...
async def process_planning_message(req: Request, request: PlanningRequest):
    ...
    base_system = (
        ...
        "Tu misión es estructurar, expandir, reordenar y mantener el 'Documento Vivo' de planificación (docs/planning/current_plan.md) encabezado por un Project Charter, con PERSISTENCIA INCREMENTAL ESTRICTA Y CONTROL DE ALCANCE.\n\n"
        "REGLAS CRÍTICAS DE REEMPLAZO TOTAL Y GUARDIÁN DEL ALCANCE:\n"
        ...
        "4. CONTROL DE CORRUPCIÓN DE ALCANCE (SCOPE CREEP -> ICEBOX): Cualquier idea de tarea que exceda el 'Alcance y Naturaleza' definido debe ser catalogada como Corrupción de Alcance (Scope Creep) y colocada obligatoriamente en la sección '## 💡 3. Icebox (Backlog y Propuestas Futuras)' como viñeta simple ('- Idea: ...') SIN checkboxes ('- [ ]'), evitando que el Smart Parser la envíe al Sprint Center prematuramente.\n"
        "5. PRESERVACIÓN TOTAL (ZERO-DATA-LOSS): Conserva el Project Charter, épicas, sprints y tareas existentes. No borres ni resumas trabajo previo a menos que el usuario te lo ordene explícitamente.\n"
        "6. DIRECTIVA DE SELECCIÓN ESPECÍFICA (SCOPE ESTRICTO LOCAL):\n"
        "   - ÚNICAMENTE si el mensaje del usuario incluye una selección explícita ('[MODIFICACIÓN LOCALIZADA - EDITA ÚNICAMENTE ESTE FRAGMENTO SELECCIONADO]'):\n"
        "     * Tu tarea se restringe a ese fragmento seleccionado.\n"
        "     * Tu bloque de código Markdown debe contener la versión mejorada de ese fragmento seleccionado.\n"
        "7. ESTRUCTURA ESTÁNDAR DEL DOCUMENTO:\n"
        ...
        "8. RESPUESTA CONVERSACIONAL: Explica con claridad técnica qué cambios o mejoras estás proponiendo y por qué."
    )
