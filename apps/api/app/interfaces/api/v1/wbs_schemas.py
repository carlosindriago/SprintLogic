...
class WBSSubtask(BaseModel):
    ...
    type: str | None = Field(default="Feature", description="Ticket type: 'Feature' | 'Refactor' | 'Technical Debt' | 'Security'")
    priority: str | None = Field(default="Medium", description="Priority: 'High' | 'Medium' | 'Low'")
    epic: str | None = Field(default=None, description="Epic name (e.g. 'Autenticación y Sesiones')")
    sprint: str | None = Field(default="Sprint 1", description="Assigned Sprint (e.g. 'Sprint 1', 'Backlog')")
    branch_name: str | None = Field(default=None, description="Suggested git branch name (e.g. 'feature/sl-101-user-model')")
    subtasks: list[dict] = Field(default_factory=list, description="Checklist steps e.g. [{'title': 'Step 1', 'completed': False}]")
