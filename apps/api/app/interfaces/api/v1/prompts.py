...
async def update_prompt(
    prompt_id: str, request: PromptPatchRequest, session: AsyncSession = Depends(get_db_session)
):
    prompt = await prompt_repository.get_prompt_async(session, prompt_id)
    if not prompt:
        raise HTTPException(status_code=404, detail="Prompt not found")
    ...
    req_vars = (
        list(prompt.required_variables) if isinstance(prompt.required_variables, list) else []
    )
    ...
    if missing_vars:
        raise HTTPException(
            status_code=400, detail=f"Missing required variables in current_content: {missing_vars}"
        )

    updated = await prompt_repository.update_prompt(
        session, prompt_id, request.current_content, req_vars
    )
    return updated
