...
import { Send, Download, Play, Zap, GraduationCap, Layout, Settings2, CheckCircle2, ClipboardCopy, FileInput, Save, FileCode2, Plus, Trash2, X } from "lucide-react";
...
- **Validación:** Qué pruebas realizar para asegurar que el código funciona.`;
  ...
  return (
    <div className="flex flex-col h-full w-full bg-zinc-950 text-zinc-200 overflow-hidden font-sans relative">
            ...
            <div className="h-full flex flex-col bg-[#1e1e1e]">
              ...
              <div className="flex items-center bg-zinc-900 border-b border-zinc-800 overflow-x-auto custom-scrollbar shrink-0">
                {openFiles.map((file) => (
                  <div
                    key={file}
                    className={`flex items-center gap-2 px-4 py-2 text-xs border-r border-zinc-800 transition-colors cursor-pointer group ${
                      activeFilePath === file
                  ...
                  >
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-0.5 rounded-sm hover:bg-zinc-700/50 hover:text-zinc-300 transition-all shrink-0 ml-1"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </div>
      ...
      {showOpenFileModal && (
        <div 
          className="absolute inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-center justify-center p-6"
          onClick={() => setShowOpenFileModal(false)}
        >
          <div 
            className="bg-[#18181b] border border-[#3f3f46] w-full max-w-2xl h-[70vh] rounded-xl shadow-2xl flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
              <h3 className="text-lg font-bold text-zinc-100">Explorador de Archivos</h3>
              <button onClick={() => setShowOpenFileModal(false)} className="text-zinc-400 hover:text-white p-1 rounded hover:bg-zinc-800">
                <X className="w-5 h-5" />
              </button>
            </div>
